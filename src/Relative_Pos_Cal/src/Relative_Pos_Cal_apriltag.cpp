...
#include <cstring>
using namespace std;
// Process measurements at 20 Hz (50 ms).
static constexpr double LOOP_HZ = 20;

// 相机坐标系到无人机机体系（IMU）的小偏移
static const Eigen::Vector3d POS_OFFSET(-0.023, -0.063, 0.03297);
      ...
      desired_yaw(0.0)
{
  ...
  try
  {
    ...
    // Keep the experimentally configured sensor queue length (10 frames).
    ...
    for (rs2::sensor s : dev.query_sensors())
    {
      ...
      {
        s.set_option(RS2_OPTION_FRAMES_QUEUE_SIZE, 10.0f);
      }
    }
  ...
  odom_sub_ = nh_.subscribe<sensor_msgs::Imu>(
      "/mavros/imu/data", 10, // 队列建议大一点，方便插值
...
void ObjectDetector::odomCallback(const sensor_msgs::Imu::ConstPtr &msg)
{
  ...
  // Retain the latest attitude as the interpolation fallback.
  R_w2i = q.toRotationMatrix();

  // Buffer timestamped attitudes for image-time interpolation.
  {
...
// Publish the current measurement and validity flag.
...
void ObjectDetector::processImages()
{
  ...
  try
  {
    // 1) Poll once for an available frameset.
    ...
    tag_detector.detect(I, 0.08, cam, cMo_vec);
