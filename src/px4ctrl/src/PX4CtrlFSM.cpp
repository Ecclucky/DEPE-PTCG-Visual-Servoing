...
PX4CtrlFSM::PX4CtrlFSM(Parameter_t &param_, LinearControl &controller_) : param(param_), nh("~"), loss_target_time_count(15), controller(controller_)
{
	state = MANUAL_CTRL;
	hover_pose.setZero();
}
...
void PX4CtrlFSM::process()
{
	...
	case MANUAL_CTRL:
	{
		if (rc_data.enter_hover_mode) // Enter AUTO_HOVER on the RC mode-switch transition.
		{
	...
	case AUTO_HOVER:
	{
		...
		else if (rc_data.is_command_mode && cmd_is_received(now_time) && loss_target_time_count == 0 && !takeoff_land_data.triggered && !in_landing)
		{
			...
			{
				state = CMD_CTRL;
				des = get_cmd_des(); // Read the current external acceleration command.
...
void PX4CtrlFSM::set_hov_with_rc() // 得到hov_pose 用遥控器来决定无人机的悬停位置和偏航角
{
	...
	hover_pose(0) += (rc_data.ch[1]) * param.max_manual_vel * delta_t * (param.rc_reverse.pitch ? -1 : 1); // RC pitch channel controls the world x reference.
	hover_pose(1) += rc_data.ch[0] * param.max_manual_vel * delta_t * (param.rc_reverse.roll ? 1 : -1);		// RC roll channel controls the world y reference.
	hover_pose(2) += rc_data.ch[2] * param.max_manual_vel * delta_t * (param.rc_reverse.throttle ? -1 : 1); // RC throttle channel controls the altitude reference.
	hover_pose(3) += rc_data.ch[3] * param.max_manual_vel * delta_t * (param.rc_reverse.yaw ? 1 : -1);		// RC yaw channel controls the heading reference.
